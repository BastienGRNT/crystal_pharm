<script lang="ts">
	...
	import { catalogueFixeMeta, type CatalogueFixeModuleKey, type CatalogueFixeSiteData } from './layout.meta';
	import { orderModules } from '../../module-order';

	let { data }: { data: CatalogueFixeSiteData } = $props();

	const orderedModules = $derived(orderModules(catalogueFixeMeta, data.modules));
	...
	const sections = $derived({
		...
		team: renderTeam
	} satisfies Record<CatalogueFixeModuleKey, Snippet>);
</script>
