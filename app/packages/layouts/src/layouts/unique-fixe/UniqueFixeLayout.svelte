<script lang="ts">
	...
	import { uniqueFixeMeta, type UniqueFixeModuleKey, type UniqueFixeSiteData } from './layout.meta';
	import { orderModules } from '../../module-order';

	let { data }: { data: UniqueFixeSiteData } = $props();

	const orderedModules = $derived(orderModules(uniqueFixeMeta, data.modules));
	...
	const sections = $derived({
		...
		hero: renderHero,
		about: renderAbout
	} satisfies Record<UniqueFixeModuleKey, Snippet>);
</script>
